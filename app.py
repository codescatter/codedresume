...
@app.route("/home/website_theme", methods=["GET","POST"])
def website():
    """
    That function was register for new user
    """
    try:
        ...
        if all_theme:
            ...
            if all_sm and all_education and all_skills and all_personal:
                ...
                return render_template(f"main_templete/{temp1}/index.html", all_sm=all_sm, all_project=all_project, all_skills=all_skills, all_experience=all_experience, all_personal=all_personal, all_education=all_education)
            else:
                return render_template("sample.html", all_sm=all_sm, all_project=all_project,
                                       all_skills=all_skills, all_experience=all_experience, all_personal=all_personal,
    ...
    try:
        if request.method=="POST":
            ...
            if username:
                ...
                return redirect(url_for('website', _external=True, _scheme=secure_type))
            else:
                flash("This user is not availble! Please first login....")
                return redirect(url_for('website', _external=True, _scheme=secure_type))
        else:
    ...
    try:
        if request.method=="POST":
            ...
            if username:
                ...
                new_dict = {"username":username, "query": query, "created_on": created_on, "updated_on": updated_on}
...
def admin(table_name):
    ...
    try:
        ...
        return render_template(f"admin/{table_name}.html", all_response=all_response, all_keys=all_keys, table_name=table_name)
